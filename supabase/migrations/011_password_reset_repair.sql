-- PROJ-5 repair: re-create password reset schema objects and RPCs if they are
-- missing in the connected Supabase project. This migration is intentionally
-- idempotent so it can safely run on projects where 004_password_reset.sql was
-- already applied.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_type
    WHERE typname = 'password_reset_status'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      ...
      AND policyname = 'password_reset_tokens_select_deny'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'password_reset_tokens'
      AND policyname = 'password_reset_tokens_insert_deny'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'password_reset_tokens'
      AND policyname = 'password_reset_tokens_update_deny'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'password_reset_tokens'
      AND policyname = 'password_reset_tokens_delete_deny'
  ) THEN
...
CREATE INDEX IF NOT EXISTS idx_password_reset_tokens_user_tenant
...
BEGIN
  IF p_token_id IS NULL OR p_user_id IS NULL OR p_tenant_id IS NULL THEN
    RETURN json_build_object('finalized', false);
  END IF;
...
BEGIN
  ...
  RETURN json_build_object('cancelled', v_token_id IS NOT NULL);
END;
$$;
...
BEGIN
  IF p_token_hash IS NULL OR char_length(trim(p_token_hash)) < 32 THEN
    RETURN json_build_object('consumed', false);
  END IF;
