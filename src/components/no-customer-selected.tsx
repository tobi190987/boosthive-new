    ...
    <Card className="rounded-[2rem] border border-slate-100 dark:border-[#252d3a] bg-white dark:bg-[#151c28] shadow-soft dark:border-slate-800 dark:bg-slate-950">
      <CardContent className="flex flex-col items-center gap-5 px-6 py-12 text-center">
        ...
        <div className="space-y-2">
          <h2 className="text-xl font-semibold text-slate-950 dark:text-slate-50 dark:text-slate-50">
            Kein Kunde ausgewaehlt
          </h2>
          <p className="max-w-md text-sm leading-7 text-slate-600 dark:text-slate-300 dark:text-slate-400">
            {hasCustomers
              ? `Waehle zuerst einen Kunden im Selektor oben links aus, um ${toolName ? `die ${toolName}-Daten` : 'die Analyse-Daten'} anzuzeigen.`
        ...
        {!hasCustomers && (
          <Button asChild className="rounded-full bg-[#1f2937] text-white hover:bg-[#111827]">
            <a href="/tools/customers">Kunden verwalten</a>
          </Button>
        )}
      </CardContent>
    </Card>
