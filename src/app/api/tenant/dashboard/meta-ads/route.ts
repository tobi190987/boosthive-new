...
export async function GET(request: NextRequest) {
  try {
    ...
    const rl = checkRateLimit(`dashboard-meta-ads:${tenantId}:${getClientIp(request)}`, GSC_READ)
    ...
    const range = (parsedQuery.data.range ?? '30d') as MetaAdsDateRangeKey
    ...
    if (!credentials || !credentials.selected_ad_account_id) {
      return NextResponse.json({ connected: false, data: null, trend: null })
    }
    ...
    const snapshot = await getMetaAdsDashboardSnapshot(integration, credentials, range)
    return NextResponse.json({
      connected: true,
      data: snapshot.data,
    ...
    console.error('[dashboard/meta-ads] Fehler:', error)
    return NextResponse.json(
