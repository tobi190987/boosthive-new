...
import {
  ...
  createTikTokAdsOAuthState,
  generateTikTokAdsNonce,
...
export async function GET(request: NextRequest) {
  ...
  const rl = checkRateLimit(`tiktok-ads-connect:${tenantId}:${getClientIp(request)}`, GSC_CONNECT)
  ...
  const state = createTikTokAdsOAuthState({
    customerId: parsedQuery.data.customerId,
    tenantId,
    userId: authResult.auth.userId,
    nonce: generateTikTokAdsNonce(),
    issuedAt: Date.now(),
  })

  return NextResponse.json({ url: buildTikTokAdsAuthorizationUrl(state) })
}
