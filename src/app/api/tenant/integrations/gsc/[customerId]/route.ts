...
import { disconnectCustomerGscIntegration, getCustomerGscIntegration } from '@/lib/gsc-customer-api'
...
export async function DELETE(
  ...
  const rl = checkRateLimit(`customer-gsc-disconnect:${tenantId}:${getClientIp(request)}`, GSC_WRITE)
  ...
  await disconnectCustomerGscIntegration(integration.id)
  await recordTenantDataAuditLog({
    tenantId,
    actorUserId: authResult.auth.userId,
    actionType: 'data_delete',
    resourceType: 'gsc_customer_connection',
