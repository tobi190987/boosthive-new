...
import {
  exchangeCustomerGscCodeForTokens,
  getCustomerGscGoogleEmail,
  verifyCustomerGscOAuthState,
} from '@/lib/gsc-customer-oauth'
import { createAdminClient } from '@/lib/supabase-admin'
import { upsertCustomerGscConnection } from '@/lib/gsc-customer-api'
...
export async function GET(request: NextRequest) {
  ...
  const payload = verifyCustomerGscOAuthState(state)
  if (!payload) {
  ...
  try {
    ...
    const tokens = await exchangeCustomerGscCodeForTokens(code)
    if (!tokens.refresh_token) {
      return NextResponse.redirect(
        buildRedirectUrl(tenant.slug, payload.customerId, 'error', 'no_refresh_token')
      )
    }

    const googleEmail = await getCustomerGscGoogleEmail(tokens.access_token)
    await upsertCustomerGscConnection({
    ...
    console.error('[gsc-customer/callback] Fehler:', error)
    return NextResponse.redirect(
