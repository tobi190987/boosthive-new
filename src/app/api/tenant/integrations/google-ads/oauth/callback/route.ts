...
import { upsertGoogleAdsConnection } from '@/lib/google-ads-api'
...
function buildRedirectUrl(
  ...
  if (result === 'connected') {
    params.set('google_ads', 'connected')
  } else {
    params.set('google_ads_error', errorMsg ?? 'unknown_error')
  }
...
export async function GET(request: NextRequest) {
  ...
  const payload = verifyGoogleAdsOAuthState(state)
  if (!payload) {
  ...
  try {
    ...
    const tokens = await exchangeGoogleAdsCodeForTokens(code)
    ...
    const googleEmail = await getGoogleAdsGoogleEmail(tokens.access_token)
    ...
    console.error('[google-ads/callback] Fehler:', error)
    return NextResponse.redirect(
