...
import {
  ...
  exchangeMetaAdsCodeForToken,
  getMetaAdsUserProfile,
  verifyMetaAdsOAuthState,
} from '@/lib/meta-ads-oauth'
import { createAdminClient } from '@/lib/supabase-admin'
import { upsertMetaAdsConnection } from '@/lib/meta-ads-api'
...
function buildRedirectUrl(
  ...
  if (result === 'connected') {
    params.set('meta_ads', 'connected')
  } else {
    params.set('meta_ads_error', errorMsg ?? 'unknown_error')
  }
...
export async function GET(request: NextRequest) {
  ...
  const payload = verifyMetaAdsOAuthState(state)
  if (!payload) {
  ...
  try {
    ...
    const shortLived = await exchangeMetaAdsCodeForToken(code)
    const longLived = await exchangeForLongLivedMetaToken(shortLived.access_token)
    const profile = await getMetaAdsUserProfile(longLived.access_token)

    await upsertMetaAdsConnection({
      customerId: payload.customerId,
      userId: payload.userId,
      accessToken: longLived.access_token,
      tokenExpiry: new Date(Date.now() + longLived.expires_in * 1000).toISOString(),
    ...
    console.error('[meta-ads/callback] Fehler:', error)
    return NextResponse.redirect(
